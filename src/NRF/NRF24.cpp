...
void nrf24_w_reg(uint8_t reg, uint8_t *data, uint8_t size){
	...
	HAL_SPI_Transmit(&NRF24_SPI, &cmd, 1, NRF24_SPI_TIMEOUT);
	HAL_SPI_Transmit(&NRF24_SPI, data, size, NRF24_SPI_TIMEOUT);
...
uint8_t nrf24_r_reg(uint8_t reg, uint8_t size){
	...
	HAL_SPI_Transmit(&NRF24_SPI, &cmd, 1, NRF24_SPI_TIMEOUT);
	HAL_SPI_Receive(&NRF24_SPI, &data, size, NRF24_SPI_TIMEOUT);
...
void nrf24_set_crc(uint8_t en_crc_val, uint8_t crc_bytes){
	uint8_t data = nrf24_r_reg(CONFIG, 1);

	data &= ~(1 << EN_CRC) & ~(1 << CRCO);

	data |= (en_crc_val << EN_CRC) | (crc_bytes << CRCO);
...
uint8_t nrf24_r_status(void){
	...
	csn_low();
	HAL_SPI_TransmitReceive(&NRF24_SPI, &cmd, &data, 1, NRF24_SPI_TIMEOUT);
	csn_high();
